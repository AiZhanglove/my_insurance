...
export default class Tickets extends React.Component {
    ...
    render() {
        var self = this;
        return (
            <div className="newtickets-box-wraper">
                {self.props.gap ? <div className="btm-bl">&nbsp;</div> : ''}
                <div className="tickets-box" onClick={() => self.showTickets()}>
                    <label>红包</label>
                    <span className="tickets-value">{ self.state.hasCoupon ? self.state.coupontips > 0 ? "立减" + methods.fen2Yuan(self.state.coupontips) + "元" : "请选择" : "暂无可用"}</span>
                </div>
                <div className="tickets-listbox" ref="ticketsModal" style={{display: self.state.showModal ? "block" : "none"}}>
                    <div className="tickets-closebar">
                        <span className="tickets-closebtn" onClick={()=>self.cancel()}></span>
                    </div>
                    <div className="tickets-list-inner">
                                            ...
                                            <div className="tickets-list-content">
                                                <div className="left">
                                                    <strong className="price">
                                                        {methods.fen2Yuan(el.preferentialAmount)}
                                                    </strong>
                                                    元
                                                </div>
                                                <div className="right">
